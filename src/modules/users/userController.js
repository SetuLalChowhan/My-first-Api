...
async function registration(req, res) {
    ...
    try {
        ...
        if (existUser) return res.status(400).send("Already registered with this email");
...
async function update(req, res) {
    const {id} = req.params;
    const {username, email, password, first_name, last_name, phone_number, age, address} = req.body;
    try {
        const user = await User.findOne({
            where: {
                id,
