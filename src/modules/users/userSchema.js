...
const userDetails = {
    ...
    first_name: string().min(4, "First Name at least  4 characters").max(40, "First Name must be at most 40 character"),
    last_name: string().min(4, "last Name at least  4 characters").max(40, "last Name must be at most 40 character"),
    phone_number: number()
        .positive("Phone number must be a positive")
        .min(10, " Phone Number must be more than 10 characters"),

    age: number().positive("Age  must be a positive").min(10, " Age  must be more than 10 characters"),

    address: string().min(15, "address be at least 15 characters").max(50, "address  must be at most 60 characters"),
};
...
const userSingleUpdate = {
    ...
    email: string().email("should be a valid email address").max(100, "Email must be at most 100 characters"),
    password: string()
        .min(8, "Password must be at least 8 characters")
        .max(50, "Password must be at most 60 characters"),
    ...
    age: number().positive("Age must be a positive"),
    address: string().min(15, "address be at least 15 characters").max(50, "address  must be at most 60 characters"),
};
